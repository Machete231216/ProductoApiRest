...
//                }))
                ...
                .headers(headers -> headers.frameOptions(frame -> frame.disable())) // permitir iframes (para H2) | es para la aplicacion web
                // Esto actúa antes del controlador
                //estas son las politicas
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/auth/**","/h2-console/**").permitAll() // pública para login/register | el /** significa lo que sea despues
                        //.requestMatchers(HttpMethod.GET,"/api/productos").permitAll() // permite a todos los usuarios
                        .requestMatchers(HttpMethod.GET,"/api/**").permitAll()
//                        .requestMatchers(HttpMethod.POST,"/api/productos").authenticated()
//                        .requestMatchers(HttpMethod.PUT,"/api/productos/**").hasRole("ADMIN") // tiene que tener el rol ADMIN
    ...
    public PasswordEncoder passwordEncoder() {
        return new BCryptPasswordEncoder(); //encripta la password
    }
